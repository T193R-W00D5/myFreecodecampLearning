...
test.describe("fixture_startFrom_Home Tests", () => {
  test("should load the home page successfully @critical @smoke", async ({
    ...
    await expect(fixture_startFrom_Home.locator("h1").first()).toHaveText(
      "My learning",
    );
  ...
  test("should have working navigation links @critical", async ({
    fixture_startFrom_Home,
  }) => {
  ...
  test("should display course navigation @smoke", async ({
    fixture_startFrom_Home,
  }) => {
    // Check courses section exists
    await expect(
      fixture_startFrom_Home.locator("main section").nth(1).locator("h1"),
    ).toHaveText("My Learning Home Pages");

    // Check course link exists, is visible, and has the expected link text
  ...
  test("should have proper page structure @regression", async ({
    fixture_startFrom_Home,
  }) => {
    // Verify semantic HTML structure
    await expect(
      fixture_startFrom_Home.locator("header#idHeader"),
    ).toBeVisible();
    await expect(fixture_startFrom_Home.locator("main")).toBeVisible();
    await expect(
      fixture_startFrom_Home.locator("footer#idFooter"),
    ).toBeVisible();

    // Check favicon is loaded
    await expect(
      fixture_startFrom_Home.locator('link[rel="icon"]'),
    ).toHaveAttribute("href", "/tigercuriosity/assets/favicon/Wizard.ico");

    // Check CSS is loaded
    await expect(
      fixture_startFrom_Home.locator('link[rel="stylesheet"]'),
    ).toHaveAttribute("href", "/tigercuriosity/css/styles-tigercuriosity.css");
  });
